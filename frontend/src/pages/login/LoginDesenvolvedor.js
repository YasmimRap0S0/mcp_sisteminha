...
export function LoginDesenvolvedor() {
  const { email, setEmail, password, setPassword, messages, setMessages, handleSubmit } = useContext(LoginDesenvolvedorContexto);
      ...
      {messages.length > 0 && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="max-w-md p-6 text-center bg-white rounded-lg shadow-lg">
            ...
            <button onClick={() => setMessages([])} className="px-4 py-2 mt-4 text-white transition bg-red-500 rounded-lg hover:bg-red-600">Fechar</button>
          </div>
        </div>
      )}
        ...
        <div className="relative z-20 flex justify-center flex-1 max-w-screen-xl m-0 bg-white shadow sm:m-5 sm:rounded-md">
          <div className="relative z-20 p-0 mt-0 lg:w-1/2 xl:w-5/12 max-md:w-full ">
            <div className="relative z-20 flex w-full">
              <Link to="/login-desenvolvedor" className="flex-1 py-4 text-center font-bold text-white bg-[#7D5C9C] hover:bg-indigo-700">
                Sou desenvolvedor
              </Link>
              <Link to="/login-microempreendedor" className="flex-1 py-4 font-bold text-center bg-gray-300 hover:bg-gray-400 text-[#978F9F]">
          ...
          <div className="max-md:hidden flex-1 flex justify-center items-center text-center bg-gradient-to-b from-[#7C5C9D] to-[#3B1C58] lg:flex">
            <div className="absolute top-[22%] -translate-y-1/2 right-15 text-white w-[360px] text-lg text-left ml-10 font-semibold">
              Oportunidades incríveis te esperam! Faça login agora e encontre o seu próximo desafio.
            </div>
            <img className="mt-8 w-90 h-90" src={imagem} alt="Imagem de um desenvolvedor" />
          </div>
        </div>
      </div>
